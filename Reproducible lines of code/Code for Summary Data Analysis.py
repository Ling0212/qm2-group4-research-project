...
# We need to successively select the values for each variables, for either Jordan's or Lebanon's data contained in the corresponding csv file. In other words, we consecutively select one column of the csv file.

# The following lines of code allow us to compute for each variable the summary statistics values. We present for example the lines used in the case of the Total Energy Supply per capita in Leabanon, from 1990 to 2019.
